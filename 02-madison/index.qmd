...
b1 <- read_bullet("../data/UserStudy/Barrel 6/Bullet 1")
b2 <- read_bullet("../data/UserStudy/Barrel 6/Bullet 2")
bullet_demo <- bind_rows(b1, b2) |>
    mutate(set = 44, barrel=6,bullet = rep(1:2, each=6), land = rep(1:6, times=2)) |>
...
#| include: false
x3p_image(b1$x3p[[2]]) 
x3p_snapshot(file = "../images/h44-br6-bullet1-land2.png")

x3p_image(x3p_add_hline(bullet_demo$x3p[[2]], yintercept=bullet_demo$crosscut[2], size = 10)) 
x3p_snapshot(file = "../images/h44-br6-bullet1-land2-crosscut.png")

x3p_image(x3p_add_hline(bullet_demo$x3p[[11]], yintercept=bullet_demo$crosscut[11], size = 10))
x3p_snapshot(file = "../images/h44-br6-bullet2-land5-crosscut.png")
```


![](../images/h44-br6-bullet1-land2.png){fig-alt="A screenshot of a rendered bullet land engraved area." .fragment}
...
![](../images/h44-br6-bullet1-land2-crosscut.png){fig-alt="A screenshot of a rendered bullet land engraved area with the crosscut marked across the bottom in silver." .fragment}
...
crosscuts <- bullet_demo |> 
  dplyr::select(-source, -x3p) |> 
  tidyr::unnest(ccdata) 

crosscuts <- crosscuts |>
  dplyr::group_by(set, barrel, bullet, land) |>
  tidyr::complete(set, nesting(barrel, bullet, land), x = full_seq(x, .645), fill = list(value = NA)) |>
  tidyr::fill(crosscut, y, .direction = "downup") |>
  dplyr::arrange(set, barrel, bullet, land, x) |>
  dplyr::ungroup()

crosscut_na <- filter(crosscuts, is.na(value))

ggplot(crosscuts, aes(x = x, y = value)) + 
  geom_vline(data = crosscut_na, aes(xintercept=x), alpha = .05, color="red") + 
  geom_line() + 
...
[This turns out to be a pretty tricky problem!]{.emph .blue .fragment}
